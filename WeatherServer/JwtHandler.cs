...
{
    public class JwtHandler(IConfiguration configuration, UserManager<WorldCitiesUsers> userManager)
    {
        //Generates the token
        public async Task<JwtSecurityToken> GetTokenAsync(WorldCitiesUsers user) =>
        new(
        ...
        private async Task<List<Claim>> GetClaimsAsync(WorldCitiesUsers user)
        {
